...
class ChatService {
  ...
  Future<String> createOrGetPersonalChat({
  ...
  final userInfo = await getUserInfoForMessage();
  final currentUserName = userInfo['name'] ?? 'Пользователь';
  
  final users = [currentUser.uid, otherUserId]..sort();
  final chatId = 'personal_${users.join('_')}';
