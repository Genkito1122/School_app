...
class _MainPageState extends State<MainPage> {
  ...
  Future<void> _loadUserName(String role) async {
    ...
    try {
      ...
      if (profileDoc.exists) {
        ...
        print('_fullUserData: $_fullUserData');
        print('childIds: ${_fullUserData?['childIds']}');
      }
    } catch (e) {
  ...
  if (_userRole == 'admin') {
    ...
    if (_fullUserData != null && _fullUserData!['childIds'] != null) {
      childIds = List<String>.from(_fullUserData!['childIds']);
    }
    
    roleScreens.addAll([
      const ChatsPage(), 
      ScheduleViewPage(childId: childIds.isNotEmpty ? childIds.first : null),
      StudentGradesPage(childId: childIds.isNotEmpty ? childIds.first : null),
      StudentHomeworkPage(childId: childIds.isNotEmpty ? childIds.first : null),
    ]);
  }
