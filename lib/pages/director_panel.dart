...
class _DirectorPanelState extends State<DirectorPanel> {
      ...
      children: [
        ...
        ListTile(
          ...
          onTap: () {
            ...
            _confirmDeleteVicePrincipal(vicePrincipal);
          },
        ),
      ],
    ),
  ...
  final teacherId = teacher['uid'] ?? teacher['id'];
  final teacherName = teacher['fullName'] ?? 'Неизвестно';
  ...
  final confirm = await showDialog<bool>(
    ...
    builder: (ctx) => AlertDialog(
      ...
      content: Text(
        'Вы уверены, что хотите удалить "$teacherName" из школы?\n\n'
        'Это действие нельзя отменить. Учитель потеряет доступ к системе.'
      ),
      actions: [
        TextButton(
  ...
  if (confirm != true || _schoolData == null) return;

  try {
    setState(() {}); // Показываем загрузку

    final schoolId = _schoolData!['schoolId'];

    // 1. Удаляем учителя из коллекции teachers
    await FirebaseFirestore.instance.collection('teachers').doc(teacherId).delete();

    // 2. Удаляем из чата учителей
    final chatId = 'teachers_$schoolId';
    await FirebaseFirestore.instance.collection('chats').doc(chatId).update({
      'participants': FieldValue.arrayRemove([teacherId]),
      'participantNames.$teacherId': FieldValue.delete(),
    });

    // 3. Убираем из классов, где он классный руководитель
    final classesSnapshot = await FirebaseFirestore.instance
        .collection('classes')
    ...
    // 4. Удаляем назначения на предметы
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Учитель "$teacherName" удалён')),
      );
    }
  } catch (e) {
    print('❌ Ошибка удаления учителя: $e');
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Ошибка: $e'), backgroundColor: Colors.red),
      );
...
Future<void> _confirmDeleteVicePrincipal(Map<String, dynamic> vicePrincipal) async {
  final vpId = vicePrincipal['uid'] ?? vicePrincipal['id'];
  final vpName = vicePrincipal['fullName'] ?? 'Неизвестно';

  final confirm = await showDialog<bool>(
    context: context,
    builder: (ctx) => AlertDialog(
      title: const Text('Удалить завуча?'),
      content: Text(
        'Вы уверены, что хотите удалить "$vpName" из школы?\n\n'
  ...
  try {
    ...
    // 1. Удаляем завуча
    await FirebaseFirestore.instance.collection('vice_principals').doc(vpId).delete();

    // 2. Удаляем из чата учителей
    final chatId = 'teachers_$schoolId';
    await FirebaseFirestore.instance.collection('chats').doc(chatId).update({
      'participants': FieldValue.arrayRemove([vpId]),
      'participantNames.$vpId': FieldValue.delete(),
    });

    // 3. Обновляем роль в users
    await FirebaseFirestore.instance.collection('users').doc(vpId).update({
      'role': null,
      'profileComplete': false,
    });

    // 4. Обновляем UI
    await _loadVicePrincipals(schoolId);

    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Завуч "$vpName" удалён')),
      );
    }
  } catch (e) {
    print('❌ Ошибка удаления завуча: $e');
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Ошибка: $e'), backgroundColor: Colors.red),
      );
